export const siteConfig = {
  ...
  legal: {
    ...
    todayBonus: {
      title: 'Замовте сьогодні — отримайте бонус',
      description:
        'Оформіть замовлення через Telegram до кінця дня та отримайте від консультанта електронний довідник продукції з детальною інформацією про кожну позицію.',
      bonusName:
        'Електронний довідник продукції Herbalife Nutrition — склад, застосування та корисні поради.',
      disclaimer:
        'Пропозиція від незалежного дистриб\'ютора Max Nutrition. Не є офіційною акцією компанії Herbalife Nutrition. Бонус надається після узгодження замовлення під час консультації.',
    },
    medicalNote:
...
export function getInterestListTelegramMessage(items: InterestListTelegramItem[]): string {
  ...
  return [
    ...
    '',
    'Також цікавить бонус — електронний довідник продукції при замовленні сьогодні.',
  ].join('\n')
}
