export const siteConfig = {
  ...
  legal: {
    ...
    todayBonus: {
      title: 'Замовте протягом 24 годин — отримайте бонус',
      description:
        'Оформіть замовлення через Telegram протягом 24 годин і отримайте від консультанта електронний довідник продукції з детальною інформацією про кожну позицію.',
      bonusName:
        'Електронний довідник продукції Herbalife Nutrition: склад, спосіб застосування та корисні поради.',
      timerLabel: 'Залишилось на пропозицію',
      badge: 'Обмежена пропозиція',
      disclaimer:
        'Пропозиція від незалежного дистриб\'ютора Max Nutrition. Не є офіційною акцією компанії Herbalife Nutrition. Бонус надається після узгодження замовлення під час консультації. Таймер діє 24 години з моменту першого відвідування сайту.',
    },
    medicalNote:
...
export function getInterestListTelegramMessage(items: InterestListTelegramItem[]): string {
  ...
  return [
    ...
    '',
    'Також цікавить бонус — електронний довідник продукції при замовленні протягом 24 годин.',
  ].join('\n')
}
